...
plt.subplot(122),plt.imshow(img),plt.title('2(resize)')
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #convert to grey scale
gray = cv2.bilateralFilter(gray, 13, 15, 15)
cv2.imshow('a',gray)
cv2.imwrite(filename + "_2." + ext, gray)
plt.subplot(123),plt.imshow(gray),plt.title('3')

edged = cv2.Canny(gray, 30, 200) #Perform Edge detection
cv2.imshow('img',edged)
plt.subplot(124),plt.imshow(edged),plt.title('4(edge detection)')
cv2.imwrite(filename + "_3." + ext, edged)

contours = cv2.findContours(edged.copy(),cv2.RETR_TREE,
                                            cv2.CHAIN_APPROX_SIMPLE)
print("am ajuns", contours)
...
# cv2.imwrite(filename + "_555." + ext, new_image)
